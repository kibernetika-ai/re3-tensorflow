...
class OpenVinoRe3Tracker(object):
    def __init__(self, checkpoint_dir,re3_openvino_dir,profiler: Profiler = None):
        drv = driver.load_driver('openvino')
        self.serving = drv()
        self.serving.load_model(re3_openvino_dir, flexible_batch_size=False)
        tf.Graph().as_default()
        self._init_tf()
    ...
    def _init_tf(self):
        self.placeHolder = tf.placeholder(tf.float32, shape=((None, 1, 1024)))
        self.prevLstmState = tuple([tf.placeholder(tf.float32, shape=(None, LSTM_SIZE)) for _ in range(4)])
        self.batch_size = tf.placeholder(tf.int32, shape=())
        self.outputs, self.state1, self.state2= network.inference_p2(self.placeHolder, num_unrolls=1,batch_size=self.batch_size,prevLstmState=self.prevLstmState)



    def _run_sess(self, croppedInput, lstmState, mult=0):
        self._profiler.start(self._re3_sess_p1_profiler)
        croppedInput = np.stack(croppedInput)
        croppedInput = np.transpose(croppedInput,(0,3,1,2))
        outputs = self.serving.predict({'Placeholder': croppedInput})
        outputs = outputs['re3/fc6/Reshape']
        self._profiler.stop(self._re3_sess_p1_profiler)

        feed_dict_1 = {
            self.placeHolder: outputs,
